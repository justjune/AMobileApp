...
public class MainActivity extends AppCompatActivity {
        ...
        if (resultCode == Activity.RESULT_OK) {
            try {
                ...
                if (st != null) {
                    ...
                    ArrayList<StCity> existingCities = stcConnector.selectAll();
                    boolean exists = false;
                    for (StCity city : existingCities) {
                        if (city.getName().equals(st.getName())) {
                            exists = true;
                            break;
                        }
                    }
                    ...
                    if (exists) {
                        Toast.makeText(this, "Этот населенный пункт уже добавлен", Toast.LENGTH_SHORT).show();
                    } else {
    ...
    public void sendPOST(StCity state, StCityAdapter adapter) {
        if (!isNetworkAvailable()) {
            runOnUiThread(() -> {
                state.setTemp(getString(R.string.no_internet));
                adapter.notifyDataSetChanged();
